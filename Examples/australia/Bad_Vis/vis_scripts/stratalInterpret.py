...
def strataAnimate(width = 7, height = 3, cs = None, nstep = None, time = None, rangeX = None, rangeY = None, 
                  folder = None):
    """
    Plot temporal stratal layers.
    Parameters
    ----------
    variable: cs
        Cross-sections dataset.
    variable: nstep
        Layer step to plot the cross-section.
    variable: time
        Time of the temporal stratal layers.
    variable: rangeX, rangeY
        Extent of the cross section plot.
    variable: folder
        Folder path to the figure and animation outputs.
    """
    ...
    for k in range(len(cs)):
        ...
        plt.rc("font", size=10)
        #
        ax = fig.add_axes([0.14, 0.18, 0.8, 0.7])
        for j in range(0,cs[k].nz,nstep):
            plt.plot(cs[0].dist,cs[k].secDep[j],color='dimgrey',linewidth=0.2)
        
        plt.title('%3.1f yr'%(time[k]))
        plt.xlim( rangeX )
        plt.ylim( rangeY )
        plt.xlabel('Distance (m)')
        plt.ylabel('Depth (m)')
        fig.savefig("%s/strata%s.png"%(folder,str(k).zfill(2)), dpi=400)
        plt.close(fig)
    
    return
    
def build_ShoreTrajectory(shoreline = None, sealevel = None, cTTC='rgba(56,110,164,0.8)', 
                          cDRTC='rgba(60,165,67,0.8)', cARTC='rgba(112,54,127,0.8)', cSTC='rgba(252,149,7,0.8)'):
    """
    ...
    shore_gradient = np.gradient(shoreline)
    sea_gradient = np.gradient(sealevel)
    
    STcolors_ST = []
    
    for i in range(len(sealevel)):
        if (shore_gradient[i] == 0):
            STcolors_ST.append(cSTC)
        elif (shore_gradient[i] < 0):
            STcolors_ST.append(cTTC)
        elif (shore_gradient[i] > 0) and (sea_gradient[i] >= 0):
            STcolors_ST.append(cARTC)
        elif (shore_gradient[i] > 0) and (sea_gradient[i] <= 0):
            STcolors_ST.append(cDRTC)

    return STcolors_ST

def build_AccomSuccession(ASval = None, cR='rgba(51,79,217,0.8)', cPA='rgba(252,149,7,0.8)',
    ...
    AS_gradient = np.gradient(ASval)
    
    STcolors_AS = []
    for i in range(len(ASval)):
        if (ASval[i] > 0):
            STcolors_AS.append(cR)
        elif (ASval[i] <= 0) and (AS_gradient[i] >= 0):
...
def viewSystemsTracts(width = 8, height = 5, cs = None, layID = None, dnlay = None, color = None,
                      rangeX = None, rangeY = None, savefig = 'Yes', figname = 'systems tracts'):
    """
    Plot interpreted systems tracts on a graph.
    Parameters
    ----------
    variable: cs
        Cross-sections dataset.
    variable: outputID
        Index of loaded outputs.
    variable: dnlay
        Layer step to plot the cross-section.
    variable: colors
        Colors for the interpreted systems tracts.
    variable: rangeX, rangeY
        Extent of the cross section plot.
    variable: savefig
    ...
    ax = fig.add_axes([0.14, 0.18, 0.85, 0.76])
    lay = []
    p = 0
    xi00 = cs.dist
    for i in layID:
        lay.append(i)
        if len(lay) == 1:
            plt.fill_between(xi00,cs.secDep[0],cs.secDep[lay[p-1]], color=color[0])
        if len(lay) > 1:
            plt.fill_between(xi00,cs.secDep[lay[p-1]],cs.secDep[lay[p]], color=color[p])
        p=p+1
...
class stratalSection:
    ...
    def buildParameters(self, CS = None, Sealevel = None):
        """
        Calculate shoreline trajectory, rate of accommodation creation, rate of sedimentation.
        Parameters
        ----------
        variable : CS
    ...
    def plotSectionMap(self, title='Location of the cross-section', color=None, colorcs=None, crange=None,
                                cs=None, ctr='k',size=(8,8)):
        """
